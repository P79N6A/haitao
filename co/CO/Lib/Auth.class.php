...
class Auth{
	...
	//dale 使用快捷查询时,使用缓存的用户旧信息。
	public function check($name, $uid, $relation='or' , $quick=false ,$detail=false ) {
		if (!$this->_config['AUTH_ON'] || $uid==1)		return true;
		$authList = $this->getAuthList($uid,$quick);
		if (is_string($name)) {
		...
		if ($relation=='or' and !empty($list)) {
			return true;
		}elseif($relation=='or' and empty($list)){
			return $detail ? $this->detail($name) : false;
		}
		...
		if ($relation=='and' and empty($diff)) {
			return true;
		}elseif($relation=='and' and !empty($diff)){
			return $detail ? $this->detail($diff) : false;
		}
	...
	private function detail($arr){
		...
		$error = M()->table($this->_config['AUTH_RULE'])->field('error')->where(array('name'=>$arr[0]))->find();
		return $error['error'];
	}
	...
	//获得权限列表
	protected function getAuthList($uid,$quick) {
	...
	//获得用户资料,根据自己的情况读取数据库
	//dale 扩增了setUserInfo的方法;getUserInfo也相应修改。
	public function setUserInfo($uid,$arr){
		foreach ($arr as $k=>$v){
			self::$userinfo[$uid][$k]=$v;
		}
	}
	public function setUserTable($table){
		$this->_config['AUTH_USER'] = $table;
	}
	protected function getUserInfo($uid) {
		static $userinfo=array();
		if (self::$userinfo[$uid])	$userinfo[$uid]=self::$userinfo[$uid];
