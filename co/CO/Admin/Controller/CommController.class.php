...
class CommController extends Controller {
	...
	public $where=array('status'=>array('in','0,1')),$theParam,$groups,$wxappId,$wxappSecret,$oauth,$code,$com_ratio;
	...
	function __construct(){
		...
		//加盟商佣金计算比例
		$this->com_ratio=$this->current_user['com_ratio']?$this->current_user['com_ratio']:0;
	}
...
/*获取自消费、客户消费、销售返利、下级返利、总返利、下级平台管理分润*/
	...
	foreach ($order as $k => $v) {
		$sell_back+=intval($v['number'])*floatval($v['product_price'])*$this->com_ratio/100;
	}
...
/*获取自消费、客户消费、销售返利、下级返利、总返利*/
	public function time_get_fee($value=array(),$star_time=0,$end_time=0){
		//var_dump($list);
			...
			$join = 'qq_order_data as od ON o.id = od.order_id';
			$agorder = M('Order')->alias('o')->field($_field)->join($join)->where($agwhere)->select();
			$result['next_fee'] += $agorder[0]['all_amount'];//负责区域客户消费
...
/*****************销售返利*******************/
	public function time_sell_back($user_id,$star_time=0,$end_time=0){
		$model=M('order');
	$order=$model->field('qq_order_data.product_price,qq_order_data.number,qq_order_data.menber_rebate')->join("`qq_order_data` on `qq_order_data`.order_id=`qq_order`.id")->where(" `qq_order`.shop_id=".$user_id." and `qq_order`.status=1 and `qq_order`.pay_time between ".$star_time." and ".$end_time)->select();
	$agentarea = M('Agentarea')->where('userId='.$user_id)->find();
	//获取订单中商品的总价和返利比率（不取当前商品表中的数据是因为这样计算出来的金额有可能跟消费的金额有误差）
	$sell_back=0;
	foreach ($order as $k => $v) {
...
public function time_next_back($user_id,$star_time=0,$end_time=0){
		$sell_back=0;
		$order=M('order')->field(' `qq_order_data`.product_price,`qq_order_data`.number,`qq_order_data`.parent_rebate ')->join("`qq_order_data` on `qq_order_data`.order_id=`qq_order`.id")->where(" `qq_order`.parent_shopid=".$user_id." and `qq_order`.status=1 and `qq_order`.pay_time between ".$star_time." and ".$end_time)->select();
		foreach ($order as $k => $v) {
			$sell_back+=intval($v['number'])*floatval($v['product_price'])*$this->com_ratio/100;
		}
	return $sell_back;
