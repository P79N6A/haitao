...
/*		if($Payment['pay_fee']){
	...
	public function get_order(){
		if($_REQUEST['method']=='QueryOrder' && $_REQUEST['Key']=='8nIeiNcOnARIPxnfg2bbJFPk5EA'){
			...
			$order=M('order')->field('id,sn,shop_id,consignee,province,city,area,address,mobile,order_amount,cod_amount,invoice_title')->where('sn='.$order_sn)->find();//获取订单信息
			if($order){
				$res=array();
				$Area = M('Area')->getField('id,name');
				...
				$res['payment']=$order['cod_amount'] ? floatval($order['cod_amount'])*100:'';//这是货到付款金额,以分计
