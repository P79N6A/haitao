...
/*					$str=$_COOKIE['shop'];
				$this_shop=unserialize($str);
		if(!$this_shop['id']||$_REQUEST['shop_id']){
				if($_REQUEST['shop_id']){
					$shop_id=$_REQUEST['shop_id'] ? intval($_REQUEST['shop_id']):0;
					$shop=M('user')->field("id,shop_name")->where('id='.$shop_id." and groupid between 6 and 13")->find();
					if(!$shop){
				...
				$shop['id']=$shop['id']?$shop['id']:1;
				if(!empty($shop))$this_shop=$shop;
 				$str=serialize($this_shop);
 				setcookie("shop",$str,time()+3600*24*365);
				$_SESSION['shop_id']=$shop['id']?$shop['id']:1;
			}*/
			...
			if($_COOKIE['YP_auth']){
				...
				list($userid,$groupid, $password) = explode("-", authcode($_COOKIE['YP_auth'], 'DECODE', $qqcms_auth_key));
				$this->_userid = $userid;
    ...
    protected function wechat_login(){
		...
		$res=$this->mp->mpAuth("http://wx.les-partageurs.com",$scope='snsapi_userinfo');
		/*加密微信信息*/
		$auth_res=(array)json_decode($res);
		$wechat_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$_SERVER['HTTP_USER_AGENT']);
		$wechat_auth = authcode($auth_res['openid'].":::".$auth_res['nickname'], 'ENCODE', $wechat_auth_key);
	...
	/*登录操作*/
		$wechat_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$_SERVER['HTTP_USER_AGENT']);
		list($wechat_u['openid'],$wechat_u['nickname']) = explode(":::", authcode($_COOKIE['YP_wechat_auth'], 'DECODE', $wechat_auth_key));
