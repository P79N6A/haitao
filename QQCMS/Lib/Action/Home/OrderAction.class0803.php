...
class OrderAction extends BaseAction
{
	...
	public function ajax(){ 
		...
		if($do=='add'){
			...
			/*end*/
				...
				//计算商品行邮税
				if ($r[post_rate]>0)
				$data[direct_shipping] = (($r['member_price']*$r[post_rate])/100)*$num;
				else
				$data[direct_shipping] = $r[post_price]*$num;
					...
					case "5":
						{
							switch ($group_id['groupid']) {
								...
								case "4|6|8|9|10|11|12|13":
									...
									//计算商品行邮税
									if ($r[post_rate]>0)
									$data[direct_shipping] = (($r['gold_price']*$r[post_rate])/100)*$num;
									else
									$data[direct_shipping] = $r[post_price]*$num;
									break;
								default:
					...
					case "4":
						{
							switch ($group_id['groupid']) {
								...
								default:
									...
									//计算商品行邮税
									if ($r[post_rate]>0)
									$data[direct_shipping] = (($r['price']*$r[post_rate])/100)*$num;
									else
									$data[direct_shipping] = $r[post_price]*$num;
									break;
							}
			...
			/**/
			/*比对商品库存 by dension start*/
				$product=M($module)->field("stock,status")->where("id=".$data['product_id'])->find();
	...
	public function done(){
		...
		{
			$idata['id'] = $user['id'];
			$idata['realname'] = $_POST['identity_name'];
			$idata['identity'] = $_POST['identity'];
			M('User')->save($idata);
		}
		...
		$order['order_amount'] = $order['amount']+$order['shipping_fee']+$order['direct_total'];
		}
		...
		if($_POST['cash_pay'] == 1 && $temp_amount > 0){
			...
			if(floatval($_POST['cash_pay_amount']) > $temp_amount)
			{
				...
				$_POST['cash_pay_amount'] = floatval($_POST['cash_pay_amount']) - $temp_amount;//减去商城总额余下可用的电子现金
			}else{
		...
		{
			import("@.ORG.QRcode");
			QRcode::png($order['sn'],$order['order_qrcode'],'L',10,4,2);
		}
