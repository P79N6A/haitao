<?php
/*推荐产品页*/
class TosellAction extends BaseAction{

        function _initialize()
        { 
          parent::_initialize();
        }
        public $gh_local_id,$gh_id,$appId,$appSecret;
        public function index(){
              ...
              $pages = $page->show();
              $model=M("tosell");
              $list=$model->alias(" as a")->field("a.id,a.title,b.name,a.logo")->join(" `qq_type` as b on a.type=b.typeid")->where(" a.catid={$catid} and a.status=1")->order("a.createtime desc")->limit($page->firstRow.",".$page->listRows)->select();
        ...
        public function show(){
              ...
              ##获取带参二维码信息，二维码路径以及二维码ticket##
              $code = M("qrcode")->where("userid=".$shop['id'])->find();
              $_SESSION["parent_shopid"] = $shop['id'];
              if(empty($code['ticket'])){
                /*当用户不存在二维码时生成二维码*/
                $gh = M('wechat')->field('id,gh_id,appId,appSecret')->where(array('id'=>1))->find();
