...
class AjaxAction extends BaseAction
{
	...
	function _initialize()
    {
		...
		$this->apikey = '162ab5fa4e741afa7034d53fde762bd2';
		$this->mltusername = 'gzzs';
		$this->mltpassword = 'gzzs0808';

		if($_COOKIE['YP_auth']){
			$this->_auth = $_COOKIE['YP_auth'];
			$qqcms_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$_SERVER['HTTP_USER_AGENT']);
			list($userid,$groupid,$openid,$unionid,$mobile,$wechat_name,$realname,$email) = explode("|-|", authcode($this->_auth, 'DECODE', $qqcms_auth_key));
			$this->_userid = $userid;
		...
		if($do=='save'){
			...
			if($userid){					
				if($id){
					...
					$r = $model->save($_POST);
					if($r === false)die(json_encode(array('id'=>0)));
					$_POST['edit'] =1;				
				}else{
 ...
 /**/
	...
	public function sendSMS()
	{
		...
		if (!empty($mobile))
		{
			$smsdata = M('Smsdata')->where('mobile='.$mobile)->order('id desc')->find();
			// F('smsdata',$smsdata);
			if (!empty($smsdata) && $smsdata['endtime'] > time())
			{
				$this->ajaxReturn($smsdata['sendtime'],"信息已发送",'101');
			}
			$randCode = $this->randCode(5,1);
			$content = '欢迎注册，您的短信验证码是：'. $randCode .'，【上优舶国际时尚直购门户】';
			import("@.ORG.Sms");
			$Sms = new Sms($this->apikey,$this->mltusername,$this->mltpassword);
			$smsinfo = $Sms->sendSMS($mobile,$content);
			if (!empty($smsinfo))
			{
				$smsinfos = explode(':', $smsinfo);
				if ($smsinfos[0]=='success')
				{
					$data['msgid'] = $smsinfos[1];
					$data['mobile'] = $mobile;
					$data['sendtime'] = time();
					$data['msgcode'] = $randCode;
					$data['endtime'] = strtotime('+10 minutes');
					M('Smsdata')->add($data);
					$this->ajaxReturn($data['sendtime'],"发送成功",1);
				}
			}
