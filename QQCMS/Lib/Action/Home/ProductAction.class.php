...
/*直购*/
    ...
    public function shop_goods(){
        ...
        $pageRow = $_REQUEST['page'];
        $nextRow = (($pageRow-1)*$listRows);
        $limit = $nextRow.','.$listRows;
        $ShopcolumnData = M('ShopcolumnData');
        if (IS_AJAX)
        {
            $field="p.listorder,sp.id as sc_id,sp.slide_data_id,p.id,p.catid,p.title,p.thumb,p.url,p.en_name,p.price,p.stock,p.member_price";
            $join="qq_product_oversea as p ON sp.goods_id=p.id";
            $_where['sp.slide_data_id']=intval($slide_data_id);
            $_where['p.status']=1;
            $count = $ShopcolumnData->alias('sp')->field($field)->join($join)->where($_where)->count();
            if ($nextRow > $count)
            $this->ajaxReturn($count,'No more data',0);
            $list = $ShopcolumnData->alias('sp')->field($field)->join($join)->where($_where)->limit($limit)->order('p.listorder ASC,p.id DESC')->select();

            foreach ($list as $ks => $lt) {
                $property_count=M('ProductProperty')->where("product_oversea_id=".intval($lt['id']))->count();
                if($property_count>0){
                    $property_stock=M('ProductProperty')->where("product_oversea_id=".intval($lt['id']))->sum("stock");
                    $list[$ks]['stock']=$property_stock;
                }
            }

            $this->ajaxReturn($list,$count,1);
        }
        $this->display();
    }
...
/*类别商品*/
    public function type_goods(){
        ...
        $where['catid']=36;//默认商城商品catid为5
        $where['status']=1;
        $where['typeid']=$_REQUEST['typeid']? $_REQUEST['typeid']:0;
        ...
        if (IS_AJAX)
        {
            ...
            $nextRow = (($pageRow-1)*$listRows);
            $limit = $nextRow.','.$listRows;
            $count = M('Product_oversea')->where($where)->count();
            if ($nextRow > $count)
            $this->ajaxReturn($count,'No more data',0);

            $list = $this->ajax_pro_list($where,$order,$limit);
...
/*商品评价*/
    public function appraise(){
        if($_POST['sub']){
            ...
            if($res)$this->success("感谢您的评价。");else$this->error("对不起,评价失败了。");
            exit();
        }
        $this->check_shop();//检查是否有商家id
...
/*公用商品列表方法*/
    public function ajax_pro_list($where=array(),$order,$limit){
        $product=M('Product_oversea');
        $list=$product->field("id,catid,title,thumb,url,en_name,price,member_price,gold_price,stock")->where($where)->order($order)->limit($limit)->select();
        foreach ($list as $ks => $lt) {
            $property_count=M('ProductProperty')->where("product_oversea_id=".intval($lt['id']))->count();
            if($property_count>0){
                $property_stock=M('ProductProperty')->where("product_oversea_id=".intval($lt['id']))->sum("stock");
