...
class BaseAction extends Action
{
    ...
    protected function wechat_login(){
		...
		/*加密微信信息*/
		$wechat_auth = authcode($auth_res, 'ENCODE', $wechat_auth_key);
	...
	/*登录操作*/
		...
		if(!empty($wechat_u)){
		 ...
		 if(empty($authInfo)){
		 	//自动注册
			$auth_info=$this->mp->getUserInfo($wechat_u);
			$con['wechat_openid']=$wechat_u;
			$con['wechat_pic']=$auth_info['headimgurl'];
			$con['wechat_name']=$auth_info['nickname'];
			$con['sex']=$auth_info['sex'];
			...
			$qqcms_auth = authcode($authInfo['id']."-".$authInfo['groupid']."-".$authInfo['password'], 'ENCODE', $wechat_auth_key);
