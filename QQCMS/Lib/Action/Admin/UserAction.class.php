...
public function get_order($user_id){
	...
	$list=$order->field(" `qq_order`.*,`qq_user`.realname,`qq_user`.wechat_name ")->join(" `qq_user` on `qq_user`.id=`qq_order`.userid ")->where(" `qq_order`.shop_id='{$user_id}' and `qq_order`.status=2 ")->select();
	return $list;
}
