 ...
 */
class CatimgAction extends AdminbaseAction {
	...
	function ad_index(){
		 $qq=D('Slide_data');
		$list = D('Slide_data')->field(" `qq_slide_data`.*,`qq_slide`.name ")->join(" `qq_slide` on `qq_slide`.id=`qq_slide_data`.fid ")->where(' `qq_slide_data`.fid in(1,3,4)')->order(" `qq_slide_data`.listorder ASC ,`qq_slide_data`.id DESC ")->select();
		$this->assign ( 'list', $list );
		$this->display();
	}
	function index(){
		 $qq=D('Slide_data');
		$list = D('Slide_data')->field(" `qq_slide_data`.*,`qq_slide`.name ")->join(" `qq_slide` on `qq_slide`.id=`qq_slide_data`.fid ")->where(' `qq_slide_data`.fid=2 ')->order(" `qq_slide_data`.listorder ASC ,`qq_slide_data`.id DESC ")->select();
		$this->assign ( 'list', $list );
		$this->display();
	}
	function addpic(){ 
	...
	function edit_goods(){
			...
			/*查找栏目标题*/
			$slide_data=M('slide_data')->where('id='.$slide_data_id)->find();
			$this->assign("slide_data",$slide_data);
			/*找出已选商品*/
			$sql="SELECT a.*,b.id as col_id from `qq_product` as a left join `qq_shopcolumn_data` as b on a.id=b.goods_id  where b.slide_data_id=".$slide_data_id;
			...
			/*找出该栏目下所有适合的商品*/
			$sql="SELECT * from `qq_product` where catid=5 and status=1";
			$query=mysql_query($sql);
			$info_res=array();
			while ($row = mysql_fetch_assoc($query)) {
				$info_res[]=$row;
	...
	public function goods_add(){
		...
		if($goods_id && $slide_data_id ){
			...
			$res=M('shopcolumn_data')->add($data);
	...
	public function goods_del(){
			...
			if($column_id){
				$res=M("shopcolumn_data")->where("id=".$column_id)->delete();
			}
			if($res){
				$this->success("移除成功");
			}else{
