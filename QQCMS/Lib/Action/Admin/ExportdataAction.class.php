...
class ExportdataAction  extends AdminbaseAction{
	...
	$this->end_time+=3600*24;//取第二天凌晨0时
	}
...
$objPHPExcel = $objReader->load($_FILES['excel']['tmp_name']); //$filename可以是上传的文件，或者是指定的文件
$sheet = $objPHPExcel->getSheet(0); 
$highestRow = $sheet->getHighestRow(); // 取得总行数 
$highestColumn = $sheet->getHighestColumn(); // 取得总列数
$k = 0;
//循环读取excel文件,读取一条,插入一条
//content:涂料钛白含量,例0就是0%,type:1为内墙，2为外墙,$flat:导入的excel表有3种格式
$data=array();
for($j=2;$j<=$highestRow;$j++)
{
$data[$k]['sn'] = $objPHPExcel->getActiveSheet()->getCell("E".$j)->getValue();//获取A列的值
$data[$k]['proname'] = $objPHPExcel->getActiveSheet()->getCell("I".$j)->getValue();//获取B列的值
$data[$k]['num'] = $objPHPExcel->getActiveSheet()->getCell("J".$j)->getValue();//获取B列的值
$k++;
	}
foreach ($data as $key => $value) {
	$count=M("product")->field("id,title,thumb,url,member_price,gold_price,menber_ratio,gold_ratio,menber_rebate,parent_rebate")->where("title like '%".$value['proname']."%'")->find();
	$data[$key]["product"]=$count;
	$order=M("order")->field("id,userid,amount")->where("sn=".$value['sn'])->find();
	$data[$key]["order"]=$order;
	$data[$key]["total_pro"]=$count['gold_price']*intval($value['num']);

	$con=array();
	$con['userid']=$data[$key]["order"]['userid'];
	$con['order_id']=$data[$key]["order"]['id'];
	$con['moduleid']=3;
	$con['product_id']=$data[$key]["product"]['id'];
	$con['product_thumb']=$data[$key]["product"]['thumb'];
	$con['product_name']=$data[$key]["product"]['title'];
	$con['product_url']=$data[$key]["product"]['url'];
	$con['product_price']=$data[$key]["product"]['gold_price'];
	$con['price']=$data[$key]["total_pro"];
	$con['number']=$value['num'];
	$con['menber_rebate']=$data[$key]["product"]['menber_rebate'];
	$con['parent_rebate']=$data[$key]["product"]['parent_rebate'];
	$con['ratio']=$data[$key]["product"]['gold_ratio'];
	M("order_data")->add($con);
}
var_dump($data);exit();
	$this->success("成功");
}
	...
	$this->list=M("user")->alias("a")->field("a.id,a.parent_id,a.groupid,a.receipt,a.realname,a.mobile,a.beshop_time,a.status,a.shop_name,a.wechat_name,a.wechat_openid,b.bank_name,b.account_name,b.account_number")->join(" `qq_beshop` as b on a.id=b.userid ")->where("a.groupid between 6 and 13")->order(" a.beshop_time desc")->select();
		...
		foreach ($this->list as $key => $value) {
			...
			$data[$key][]=number_format($value['receipt'],2);
			$data[$key][]=number_format($value['manage'],2);
			$data[$key][]=number_format($value['self_fee'],2);
			$data[$key][]=number_format($value['next_fee'],2);
			$data[$key][]=number_format($value['scale'],2);
			$data[$key][]=number_format($value['sell_back'],2);
			$data[$key][]=number_format($value['next_sell_back'],2);
			$data[$key][]=number_format($value['next_splitt'],2);
			$data[$key][]=number_format($value['total_back'],2);
...
protected function next_back($user_id){
		$sell_back=0;
			$order=M('order')->field(' `qq_order_data`.product_price,`qq_order_data`.number,`qq_order_data`.parent_rebate ')->join("`qq_order_data` on `qq_order_data`.order_id=`qq_order`.id")->where(" `qq_order`.parent_shopid=".$user_id." and `qq_order`.status=2 and `qq_order`.pay_time between ".$this->star_time." and ".$this->end_time)->select();
			foreach ($order as $k => $v) {
