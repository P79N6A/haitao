...
$.extend($.validator, {
	...
	prototype: {
		...
		sendsValue: function(element) {
			return $.data(element, "sendsValue") || $.data(element, "sendsValue", {
				old: null,
				valid: true,
				message: this.defaultMessage( element, "sendsms" )
			});
		}
	},
	...
	methods: {
		...
		sendsms: function(value, element, param) {
			if ( this.optional(element) )
				return "dependency-mismatch";
			var _mobile = element.alt;
			var _time = element.id;
			var previous = this.sendsValue(element);
			if (!this.settings.messages[element.name] )
				this.settings.messages[element.name] = {};
			previous.originalMessage = this.settings.messages[element.name].sendsms;
			this.settings.messages[element.name].sendsms = previous.message;
			...
			var validator = this;
			this.startRequest(element);
			var data = {'mobile':_mobile,'time':_time};
			data[element.name] = value;
			$.ajax($.extend(true, {
				url: param,
				mode: "abort",
				...
				data: data,
				success: function(response) {
					validator.settings.messages[element.name].sendsms = previous.originalMessage;
					var valid = response === true;
					if ( valid ) {
						var submitted = validator.formSubmitted;
						validator.prepareElement(element);
						...
						var errors = {};
						var message = response || validator.defaultMessage( element, "sendsms" );
						errors[element.name] = previous.message = $.isFunction(message) ? message(value) : message;
						validator.showErrors(errors);
					}
					previous.valid = valid;
