...
class Wechat extends Think {
	...
	final public function subscribe($data,$even_data){
		...
		##判断是否第一次关注##
		$where_f['openid'] = $data['FromUserName'];
		$wechat_follow = M('WechatFollow')->field('id,openid')->where($where_f)->find();
		$first_subscribe = 0;
		if (empty($wechat_follow))
			$first_subscribe = 1;


		//写入用户表，默认注册
		$us_where['wechat_openid']=$data['FromUserName'];
		$user = M("user")->field("id,parent_id,cash_use")->where($us_where)->find();
		$key=$even_data['EventKey'];//前缀qrscene_
		$parentid=explode("_", $key);
		if(!$parentid)$parentid[1]="0"; 
		$check_group=M("user")->field("groupid,wechat_name")->where("id=".$parentid[1])->find();
		...
		if(!empty($user)){
			...
			if ($first_subscribe)
				$con['cash_use'] = $user['cash_use'] + 100;//首次关注送100元
		...
		if ($first_subscribe)
		$where = array('varname'=>"wechat_scribe_award",'groupid'=>2,'lang'=>1);
		else
		$where = array('varname'=>"wechat_scribe",'groupid'=>2,'lang'=>1);
			
		$content = M('config')->field('value')->where($where)->find();
		$repaly_content['type'] = "text";
		$message = str_replace('{wechat_name}',$check_group['wechat_name'],$content['value']);
		$repaly_content['description'] = $message;
	 ...
	 * 2.根据 公众号 设置，响应信息给微信服务器
	 */
	final public function scan($data,$even_data)
	{
		//扫码逻辑请写在这里
		$user_info = $this->mp->getUserInfo($data['FromUserName']);
		//写入用户表，默认注册
		$us_where['wechat_openid'] = $data['FromUserName'];
		$user = M("user")->field("id,parent_id,cash_use")->where($us_where)->find();
		$parentid = $even_data['EventKey'];//运营商ID
		if(!$parentid) $parentid = "0"; 
		$check_group = M("user")->field("groupid,wechat_name")->where("id=".$parentid)->find();
		if(empty($check_group) || intval($check_group['groupid']) < 6 || intval($check_group['groupid']) > 13){
			$parentid = "0";//如果不是微店上级就变为总的
		}
		// if($parentid[1]==1)$parentid[1]="0";//如果是1则上级是大后台
		//file_put_contents("test.txt", $parentid[1]);
		if(!empty($user)){
			...
			$con['wechat_pic'] = $user_info['headimgurl'];
			$con['sex'] = $user_info['sex'];
			if($parentid != null) $con['parent_id'] = $parentid;
			$con['id'] = $user['id'];
			M("user")->save($con);
		}else{
			$con['wechat_pic'] = $user_info['headimgurl'];
			...
			$con['groupid'] = 3;
			$con['newAccount'] = 1;
			$con['cash_use'] = 100; //成为新用户送100元
			if($parentid != null)$con['parent_id'] = $parentid;
			$firstuser = M("user")->add($con);
		}

		//最好能先回复消息给关注者 写入数据库的操作 在后台运行
		$repaly_content['type'] = "text";
		if ($firstuser)
		$where = array('varname'=>"wechat_scan",'groupid'=>2,'lang'=>1);
		else
		$where = array('varname'=>"wechat_scan_award",'groupid'=>2,'lang'=>1);
		$content = M('config')->field('value')->where($where)->find();
		$message = str_replace('{wechat_name}',$check_group['wechat_name'],$content['value']);
		$repaly_content['description'] = $message;
		return $repaly_content;
