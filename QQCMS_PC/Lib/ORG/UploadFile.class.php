...
{//类定义开始
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    private function upload_oss_save($file){
        $filename = $file['savepath'].$file['savename'];
        /*保存文件至OSS*/
        import ('@.Oss.Aliyun');
        $file_charset=explode('/',auto_charset($filename,'utf-8','gbk'));
        $dir_name=$file_charset[1].'/'.$file_charset[2];
        $object_name=$file_charset[1].'/'.$file_charset[2].'/'.$file_charset[3];
        $aliyun = new Aliyun($dir_name,$object_name,$file['tmp_name']);

        $filename1 = explode('./',$filename);
        $bucket=$aliyun->bucket;
        $endpoint=$aliyun->endpoint;
        $full_filename='http://'.$bucket.'.'.$endpoint.'/'.$object_name;

        if(!$this->uploadReplace && is_file($full_filename)) {
            // 不覆盖同名文件
            $this->error    =   '文件已经存在！'.$full_filename;
            return false;
        }
        // 如果是图像文件 检测文件格式
        if( in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png','swf')) && false === getimagesize($file['tmp_name'])) {
        ...
        $full_filename=$res['header']['_info']['url'];
        $savepath=substr($file['savepath'], 1);
        $file['savepath']='http://'.$bucket.'.'.$endpoint.$savepath;
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            $image=getimagesize($full_filename);
            if(false !== $image) {
                //是图像文件生成缩略图
                $thumbWidth     =   explode(',',$this->thumbMaxWidth);
                $thumbHeight        =   explode(',',$this->thumbMaxHeight);
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    $thumbname  =   $thumbPath.$thumbPrefix[$i].substr($realFilename,0,strrpos($realFilename, '.')).$thumbSuffix[$i].'.'.$file['extension'];
                    Image::thumb($full_filename,$thumbname,'',$thumbWidth[$i],$thumbHeight[$i],true);
                }
                if($this->thumbRemoveOrigin) {
                    // 生成缩略图之后删除原图
    ...
    public function upload($savePath ='')
    {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                //保存上传文件,直接上传至阿里云OSS云存储
                $oss_save=$this->upload_oss_save($file);
                if(!$oss_save) return false;
