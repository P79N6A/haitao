...
class CatimgAction extends AdminbaseAction {
	...
	function edit_goods(){
			...
			/*找出已选商品*/
			$join = 'qq_product_oversea as a ON a.id = b.goods_id';
			$where['b.slide_data_id'] = $slide_data_id;
			$where['a.status'] = 1;
			$has_info = M('ShopcolumnData')->alias('b')->field('a.*,b.id as col_id,b.listorder as sort')->join($join)->where($where)->order("b.listorder ASC,b.id DESC")->select();
