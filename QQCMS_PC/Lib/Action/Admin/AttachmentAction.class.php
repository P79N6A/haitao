...
class AttachmentAction extends  Action {
		...
		if(!$this->userid){
			if($this->isadmin){
				...
				if($_COOKIE['YP_auth']){
					...
					$qqcms_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$HTTP_USER_AGENT);
	...
	public function index(){
		...
		$enupsetup = authcode($auth, 'DECODE', $qqcms_auth_key);
	...
	public function upload(){
        ...
        if (!$upload->upload()) { 
			...
			$data['filepath'] = __ROOT__.substr($uploadList[0]['savepath'].$uploadList[0]['savename'],0);
