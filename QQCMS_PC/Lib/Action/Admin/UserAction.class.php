...
class UserAction extends AdminbaseAction {
	...
	function index(){
		import ( '@.ORG.Page' );
		/*$strtotime = strtotime('-2 days -6 hours');
		$_strtotime = strtotime('-6 hours');
		$_where['createtime'] = array(array('gt',$strtotime),array('lt',$_strtotime));
		$_where['groupid'] = 4;
		$userlist = M('User')->where($_where)->select();
		$userdata['groupid'] = 4;
		foreach ($userlist as $key => $value) {
			$_where['id'] = $value['id'];
			$userdata['wechat_name'] = $value['mobile'];
			$userdata['password'] = sysmd5('123456');
			M('User')->where($_where)->data($userdata)->save();
			$adwhere['userid'] = $value['id'];
			$adwhere['isdefault'] = 1;
			$address_user = M('User_address')->where($adwhere)->find();
			if (!empty($address_user))
			{
				$address_data['id'] = $address_user['id'];
				$address_data['userid'] = $value['id'];
				$address_data['province'] = $value['province'];
				$address_data['city'] = $value['city'];
				$address_data['area'] = $value['area'];
				$address_data['consignee'] = $value['realname'];
				$address_data['mobile'] = $value['mobile'];
				$address_data['address'] = $value['address'];
				$address_data['isdefault'] = 1;
				M('User_address')->save($address_data);
			}
			else
			{
				$address_data['userid'] = $value['id'];
	...
	function insert(){
		$user=$this->dao;
		$_POST['lastrecharge_time'] = time();
		$_POST['wechat_name'] = $_POST['mobile'];
		// print_r($_POST);exit;
		$_POST['password'] = sysmd5('123456');
		...
		if($data=$user->create()){
			if(false!==$user->add()){
				...
				{
					$address_data['id'] = $address_user['id'];
					$address_data['userid'] = $uid;
					$address_data['province'] = $_POST['province'];
					$address_data['city'] = $_POST['city'];
					$address_data['area'] = $_POST['area'];
					$address_data['consignee'] = $_POST['realname'];
					$address_data['mobile'] = $_POST['mobile'];
	...
	public function agentArea()
	{
		...
		$list = $this->_list('User', $where);
		$agentlist = array();
		foreach ($list as $key => $value) {
			$aglist = M('Agentarea')->where('userId='.$value['id'])->find();
			if ($aglist)
			{
				$agentlist[$value['id']] = $aglist;
				if (is_string($aglist['agentarea']))
				{
					$agar = explode(',',$aglist['agentarea']);
					$territory = '['.$Area[$agar[0]].'-'.$Area[$agar[1]].'-'.$Area[$agar[2]].']';
					$list[$key]['agentarea'] = $territory;
				}
				$list[$key]['commission'] = $aglist['commission'];
				##获取该区域下所获得的佣金##
				$ag_where['od.userid'] = $value['id'];
				$ag_where['od.agentarea'] = $aglist['agentarea'];
				$join = 'qq_order as o ON od.order_id = o.id';
				$field = 'od.userid,od.order_id,od.agentarea,od.area_com,o.pay_status,o.status';
				$ag_info = M('Order_data')->alias('od')->field($field)->join($join)->where($ag_where)->select();
				if ($ag_info['pay_status'] == 2 && $ag_info['status'])
				{
					$num_com = 0;
					foreach ($ag_info as $k => $v) {
						$num_com += $v['area_com'];
					}
					$ag_data['id'] = $aglist['id'];
					$ag_data['com_price'] = $num_com;
					M('Agentarea')->save($ag_data);
					$list[$key]['com_price'] = $num_com;
				}
			}
		}
	...
	public function get_menber_fee($list=array()){
		...
		foreach ($temp as $k => $v) {
			if($v['pay_status']==2 && $v['cash_pay_status']!=1 && ($v['wechat_amount']>0 || $v['allinipay_amount']>0)){
				//微信支付订单
				$list[$key]['self_fee']+=floatval($v['order_amount']);
			}elseif($v['pay_status']==2&&$v['cash_pay_status']==1&&$v['wechat_amount']>0){
				//电子现金支付一部分
				$list[$key]['self_fee']+=floatval($v['order_amount']);
				$list[$key]['online_cash']+=floatval($v['cash_coupon']);
			}elseif($v['cash_pay_status']==1&&$v['order_amount']==$v['cash_coupon']){
				//电子现金支付全部金额
				$list[$key]['self_fee']+=floatval($v['order_amount']);
				$list[$key]['online_cash']+=floatval($v['cash_coupon']);
			}
...
/*获取自消费、客户消费、销售返利、下级返利、总返利、下级平台管理分润*/
	public function get_fee($list=array()){
		...
		foreach ($list as $key => $value) {
			...
			$order=M('order')->field('sum(amount) as fee')->where("userid=".$value['id']." AND status=1 AND pay_status=2")->select();
			$result['self_fee']=$order[0]['fee'];//自消费
			$order=M('order')->field(' sum(amount) as fee ')->where(" shop_id=".$value['id']." AND status=1 AND pay_status=2")->select();
...
/*****************销售返利*******************/
	...
	$order=M('order')->field('qq_order_data.product_price,qq_order_data.number,qq_order_data.menber_rebate')->join("`qq_order_data` on `qq_order_data`.order_id=`qq_order`.id")->where(" `qq_order`.shop_id=".$user_id." and `qq_order`.status=1 and `qq_order`.pay_status=2")->select();
	$agentarea = M('Agentarea')->where('userId='.$user_id)->find();

	//加盟商佣金计算比例
	$current_user=M('user')->field('*,`realname` as nickname')->find($user_id);
	$com_ratio=$current_user['com_ratio']?$current_user['com_ratio']:0;

	//获取订单中商品的总价和返利比率（不取当前商品表中的数据是因为这样计算出来的金额有可能跟消费的金额有误差）
	$sell_back=0;
	foreach ($order as $k => $v) {
		$sell_back+=intval($v['number'])*floatval($v['product_price'])*$com_ratio/100;
	}
...
public function next_back($user_id){
		$sell_back=0;
		$order=M('order')->field(' `qq_order_data`.product_price,`qq_order_data`.number,`qq_order_data`.parent_rebate ')->join("`qq_order_data` on `qq_order_data`.order_id=`qq_order`.id")->where(" `qq_order`.parent_shopid=".$user_id." and `qq_order`.status=1 and `qq_order`.pay_status=2")->select();
...
/*end*///二维码
	public function get_code($id){
		...
		/*当用户不存在二维码时生成二维码*/
		$gh = M('wechat')->field('id,gh_id,appId,appSecret')->where('id=1')->find();
...
protected function download_qrcode($url){
	...
	function checkusername(){
		...
		if(empty($userid)){
			...
			//判断用户名是否已经使用
			if($this->dao->where("id!={$userid} and username='{$username}'")->find()){
				 echo 'false';
			}else{
				echo 'true';
			}
