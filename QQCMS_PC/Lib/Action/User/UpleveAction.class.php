...
class UpleveAction extends BaseAction{
	...
	public function recharge(){
		...
		if (!empty($payInfo['wxdata']))
		{
			...
			$html .= '<button type="submit" class="btn btn-primary btn-block" onClick="getWxpay('.$wxdata['id'].','.$wxdata['type'].');">微信支付</button>';
		}

		if ($payInfo['tldata']){
			if ($payInfo['tldata']['gateway_url'])
			{
				$pay_acturl_m = $payInfo['tldata']['gateway_url'];
			...
			$html .= '<button type="submit" class="btn btn-danger btn btn-block">手机银行支付</button>';
			$html .= '</form>';
		}
		$res['data'] = $html;
		$res['fee'] = $fee;
