...
class UpleveAction extends BaseAction{
	...
	public function recharge(){
		...
		$html .= '<button type="submit" class="btn btn-warning btn btn-block pay_button">进入支付</button>';
		$html .= '</form>';
		$res['data'] = $html;
		$res['fee'] = $fee;
		$res['status'] = 1;
	...
	protected function get_rechargeurl($userid=0,$fee=0){
		...
		if(!empty($receipt_order)){
			...
			$url = $this->getAllinpay($Payment['pay_code'],$receipt_order,$Payment);//获取到支付信息
		}else{//生成订单
			$order['userid'] = $userid;
			...
			if($orderid){
				...
				$url = $this->getAllinpay($Payment['pay_code'],$order,$Payment);//获取到支付信息
