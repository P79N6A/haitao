...
{
	public $wxappId,$wxappSecret,$oauthcode,$oauth,$dao;
	...
	function _initialize()
    {
		...
		$this->dao = M('User');
		$this->assign('bcid',0);
		$wxgh = M("wechat")->field("appId,appSecret")->where("id=3")->find();
		if (!$wxgh) {$this->error("应用信息未配置");exit;}
		$this->wxappId = $wxgh['appId'];
		$this->wxappSecret = $wxgh['appSecret'];
    }
    function index()
    {
		if($this->_userid){		
	...
	function wechatlogin()
	{
		...
		$user = $this->dao->where(array("wechat_name"=>$wechatuser['nickname']))->find();
		...
		if($user){
			...
			$qqcms_auth = authcode($user['id']."-".$user['groupid']."-".$user['password'], 'ENCODE', $qqcms_auth_key);
			...
			cookie('username',$user['username'],$cookietime);
			cookie('groupid',$user['groupid'],$cookietime);
			cookie('userid',$user['id'],$cookietime);
			cookie('email',$user['email'],$cookietime);
