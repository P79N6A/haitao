...
if(defined('APP_PATH')!='./QQCMS') exit("Access Denied");
class AdminbaseAction extends Action
{
	...
	{	
		...
		if (C ( 'USER_AUTH_ON' ) && !in_array(MODULE_NAME,explode(',',C('NOT_AUTH_MODULE')))) {
			...
			if (! RBAC::AccessDecision ('Admin')) {
				...
				// 没有权限 抛出错误
				// var_dump(C ( 'RBAC_ERROR_PAGE' ));exit;
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'', './QQCMS/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,'./QQCMS/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,'./QQCMS/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			$r=$this->buildHtml($filename,$dir,'./QQCMS/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index()
    {
		...
		$r=$this->buildHtml('index','./'.$lang,'./QQCMS/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		if($r) return true;
    }
 ...
 /**/
    ...
    /**
     * 排序，二次附加函数 排序字段为：listorder
     */
    public function private_listorders() {
        $model=$_POST['model'];
        $listorder=$_POST['listorders'];
        $com_model=M($model);
        $data['id']=intval($_POST['id']);
        $data['listorder']=$listorder?intval($listorder):0;
        $status=$com_model->save($data);
        if($status===false)
            $this->error("排序更新失败！");
        else
            $this->success("排序更新成功！");
