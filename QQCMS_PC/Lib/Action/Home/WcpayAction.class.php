...
class WcpayAction extends Action
{
	...
	public function index(){
		...
		####判断支付产品类型#########
		if (strpos($this->xml_data['ProductId'],'product') !== false)
		{
			$ProductIdarr = explode("%7C",$this->xml_data['ProductId']);
		}
		
		if (isset($ProductIdarr) && is_array($ProductIdarr))
		{
			$act = 'anweisung';
			$order=M("order")->where(" status=0 and id=".$ProductIdarr[1])->find();
		...
		$this->setParameter("notify_url", "http://wx.les-partageurs.com/index.php?a=".$act."&m=Getresult");
		...
		if ($wechat_amount)
		{
			$order_fee=$wechat_amount*100;
			if ($order['userid']==5209) $order_fee=1;
			//$order_fee=1;
		}
		
		$this->setParameter("total_fee", (string)$order_fee); //wechat_amount
