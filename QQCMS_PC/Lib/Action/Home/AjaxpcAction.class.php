...
class AjaxpcAction extends BaseAction
{
 ...
 /**/
	...
	public function catch_colu(){
		...
		if($columntype_id){
			/*找出该栏目下所有适合的图片*/
			$sql="SELECT a.* from `qq_pcslide_data` as a left join `qq_pcslide` as b on a.fid =b.id left join `qq_shopcolumnpc_type` as c on c.slide_id=b.id where c.id=".$columntype_id." and a.status=1";
			...
			/*找出该栏目已选图片*/
			if($columntype_id){
				$sql="SELECT b.* from `qq_shopcolumnpc` as a left join `qq_pcslide_data` as b on a.slid_data_id=b.id where a.columntype_id=".$columntype_id." and a.uid=".$uid;
				$query=mysql_query($sql);
