...
//购买最多
	public function sells(){
        $this->check_shop();//检查是否有商家id
        $count=M("order_data")->alias("a")->join(" `qq_order` as b on a.order_id=b.id ")->join(" `qq_product` as c on a.product_id=c.id ")->where(" b.pay_status=2 and c.catid=5 and c.status=1 ")->group("a.product_id")->count();
        $listRows =  10;    
        $page = new Page ( $count, $listRows );
        $pages = $page->show();
		$list=M("order_data")->alias("a")->field(" sum(a.number) as pro_number,c.id,c.catid,c.title,c.hits,c.thumb,c.url,c.en_name,c.price,c.member_price,c.gold_price ")->join(" `qq_order` as b on a.order_id=b.id ")->join(" `qq_product` as c on a.product_id=c.id ")->where(" b.pay_status=2 and c.catid=5 and c.status=1 ")->order("pro_number desc")->group("a.product_id")->limit($page->firstRow . ',' . $page->listRows)->select();
        $this->assign("list",$list);
        $this->assign('pages',$pages);
		$this->display("hits");
	}
