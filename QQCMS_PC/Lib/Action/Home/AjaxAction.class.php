...
class AjaxAction extends BaseAction
{
	...
	public function change_par(){
		...
		$str.="<li onclick='par_select(this)' style='padding:0px 10px 0px 10px;list-style-type:none;float:left;margin-right:5px;widtg:40px;line-height:40px;'><input type='hidden' value='0'/><span>上优舶</span></li>";
		foreach ($par as $k => $v) {
 ...
 /**/
	...
	function column_edit(){
 	...
 	{
 		$Agentarea = M('Agentarea');
 		$userid = trim($_POST['userid'] ? $_POST['userid'] : 0);
 		$province = trim($_POST['province'] ? $_POST['province'] : '0');
 		$city = trim($_POST['city'] ? $_POST['city'] : '0');
 		$area = trim($_POST['area'] ? $_POST['area'] : '0');
 		$commission = trim($_POST['commission'] ? $_POST['commission'] : '0');
 		$agentarea = trim($province.','.$city.','.$area);
 		$where['agentarea'] = $agentarea;
 		$info = $Agentarea->where($where)->find();
 		if (!empty($info) && $info['userId'] != $userid)
 		{
 			$this->ajaxReturn(null,'该区域已有代理商',0);
 		}

 		$data['agentarea'] = $agentarea;
 		$data['userId'] = $userid;
 		$data['commission'] = $commission;
 		$_where['userId'] = $userid;
 		$ag = $Agentarea->where($_where)->find();
 		if ($ag)
 		{
 			$data['id'] = $ag['id'];
 			$id = $Agentarea->save($data);
 		}
 		else
 		$id = $Agentarea->add($data);
 	
 		if ($id === false)
 			$this->ajaxReturn(null,'设置失败',0);
 		else
 			$this->ajaxReturn($id,'设置成功',1);
 	}
}
?>