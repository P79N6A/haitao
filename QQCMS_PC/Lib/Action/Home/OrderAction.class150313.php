...
class OrderAction extends BaseAction
{
	...
	public function ajax(){ 
		...
		if($do=='add'){
			$session_info = M('Online')->where("userid='".$this->_userid."'")->find();
			$r=M($module)->find($id);
			/*根据拍卖跟直购只有金会员有权限*/
			...
			$cart = $this->dao->where("product_id='{$id}' and userid='{$this->_userid}'")->find();
			if($cart){
	...
	public function doQuery()
	{
		...
		$receipt_order = $model->field('id,sn,type')->where($receipt_where)->find();
	...
	public function done(){
		...
		if($this->_userid){
			...
			if($_POST['addressid']){
				...
				$address = unserialize($_COOKIE['YP_guest_address']);//从已存储的表示中创建 PHP 的值,++存储serialize()++
			}
		...
		/*商品金额*/
		$cart = $this->dao->field(" `qq_cart`.* ")->join(" `qq_product` on `qq_product`.id=`qq_cart`.product_id ")->where(" `qq_cart`.userid='{$this->_userid}' and `qq_product`.catid!=25")->select();//只取商城商品
		...
		if(!empty($cart))
		{
			...
			/*支付方式*/
			$paymentid=empty($_POST['cash_pay'])? 0:$_POST['cash_pay'];//默认选中微支付,2为电子现金支付
...
/*		if($Payment['pay_fee']){
		...
		{
			$order['cod_amount']=$temp_amount-10000;
			$order['wechat_amount']=10000;
		}
		else
		{
			$order['cod_amount']=0;
		...
		if($orderid){
			$order['id']=$orderid;
			$order['sn'] = date("Ymd"). sprintf('%06d',$orderid).'cm'; 
			$model->save(array('id'=>$orderid,'sn'=>$order['sn']));
			foreach($cart as $key=>$r)
			{
		...
		$cart_private = $this->dao->field(" `qq_cart`.* ")->join(" `qq_product` on `qq_product`.id=`qq_cart`.product_id ")->where(" `qq_cart`.userid='{$this->_userid}' and `qq_product`.catid=25")->select();//取私人酒窖商品
		if($cart_private){
		$order_private=$order;
		...
		if($temp_amount>10000){
			$order_private['cod_amount']=$temp_amount-10000;
			$order_private['wechat_amount']=10000;
		}else{
		...
		if($order_privateid){
			...
			$order_private['sn'] = date("Ymd"). sprintf('%06d',$order_privateid).'cm'; 
					...
					if( $order['order_amount']>0 && $order['cash_coupon'] <= $user_cash['cash_use'])
					{
						...
						if($r)
						{
							...
							if(empty($order['wechat_amount']))
							{
								...
								$this->put_shipping_error($orderid,"102",$shipping_res['message']);
						...
						if($order['wechat_amount']>0)
						{
							...
							//获取当前订单支付二维码wechat_qrcode和支付宝按钮
							$order['alipay_button'] = $this->getalipay("Alipay",$order);
							$order['pay_button'] = $this->get_qrcode($orderid);
						}
					}
					else
					{					
				...
				//电子现金支付	
					...
					if( $order_private['order_amount']>0 && $order_private['cash_coupon'] <= $user_cash['cash_use']){
						...
						if($order_private['wechat_amount']>0){
							...
							//获取当前订单支付二维码wechat_qrcode和支付宝按钮
							$order_private['alipay_button'] = $this->getalipay("Alipay",$order);
							$order_private['pay_button'] = $this->get_qrcode($order_privateid);
						}
					}else{					
						/*微信支付*/
						//$order_private['pay_button']=$this->pay_data($order_private);//获取到支付按钮
		...
		//print_r($native_url_code);exit;
		
		//私人酒窖支付end
		$this->assign('order',$order);
		$this->assign('native_url_code',$native_url_code);
	...
	function get_qrcode($orderid,$type=0)
	{
		$wx_native_url = $this->create_native_url($orderid,$type);
		$native_url_code = $this->creatcode($wx_native_url);
		$qrcode['id'] = $orderid;
	...
	*/
	function create_native_url($productid,$type=0)
	{
		if ($type==0)
		{
	...
	function creatcode($url)
	{
		...
		\QRcode::png($url, './shop_qrcode/native'.$randstr.'.png', 'L',6, 2); 
		$data['status'] = 1;
		$data['info'] = "微信支付二维码";
		$data['link'] = $url;
